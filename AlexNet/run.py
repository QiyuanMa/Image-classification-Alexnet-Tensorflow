...
#训练模型
def train():
    label_name_to_num = {"worm":2,"mildew":1,"hurt":0}
    #获取所有的训练数据
    ...
    with tf.Session() as sess:
        ...
        print('mean_val_acc',mean_val_acc/num_epochs)
...
def genrate_pre_result():
    ...
    with tf.Session() as sess:
        ...
        for step in range(steps):
            ...
            #预测图片的标签
            # keep_prob 一般在大量数据训练时，为了防止过拟合，添加Dropout层，设置一个0~1之间的小数
            ...
            test_pred_labels.extend(np.round(sess.run(pred_prob)[:,2],decimals=2))
        data = pd.DataFrame({"id":test_img_ids,"label":test_pred_labels})
        data.sort_values(by="id",ascending=True,inplace=True)
        #保存结果
        data.to_csv("AlexNet_transfer.csv",index=False)
