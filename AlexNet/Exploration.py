...
def plot_label_distribution(img_labels):
    ...
    plt.title("corn distribution")
    plt.show()

#用来绘制验证集和测试集的类标分布
def val_pred_and_real_distribution(pred_labels,real_labels):
    label_num_to_name = {0:"hurt",1:"mildew",2:"worm"}
    #将数字标签转为文字标签
...
def classifiction_report_info(pred_labels,real_labels):
    #输出分类结果报告
    classification_info = classification_report(real_labels,pred_labels,target_names=["hurt","mildew","worm"])
