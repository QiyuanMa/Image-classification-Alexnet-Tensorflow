...
if __name__ == '__main__':
    # 图像数据集的路径
    ims_path='image_data_correct/'
    ims_list=os.listdir(ims_path)
    # 获取总个数
    imgnum=len(ims_list)  
    # 角二阶矩（能量）、对比度、熵、反差分矩阵（逆方差）、R,G,B、H、L、S、 TAG（0机械损伤，1霉变，2虫蛀）
    data=np.zeros((imgnum, 11)) 
    for i in range(imgnum):
        ...
        if r'hurt' in im:
            ...
            data[i, 10] = 2
        img = cv.imread('image_data_correct/'+im)    
        # 0,1,2,3 添加灰度矩阵四类特征
    ...
    data_tosave.to_csv('image_data_correct.csv')
